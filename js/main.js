/* ═══════════════════════════════════════════════════════════
   LOGAN BIRTHDAY — main.js
   ═══════════════════════════════════════════════════════════ */

/* ── CONFIG — edit these to personalise ──────────────────── */
const CONFIG = {
  name: "Nino",
  message: `Happy Birthday, Nino! 🎉
...
Happy Birthday! 🥂 This one's for you. 🥂`,
  ...
  photos: [
    { src: "images/photo1.jpg", caption: "📸 Nino — always in the moment" },
    { src: "images/photo2.jpg", caption: "✨ Beautiful moments, forever remembered" }
  ],
...
function fadeInMusic() {
  const audio = document.getElementById("bg-music");
  const vol = parseFloat(document.getElementById("volume-slider").value) / 100;
  audio.volume = 0;
  let v = 0;
  const step = () => {
    v = Math.min(v + 0.02, vol);
    audio.volume = v;
    if (v < vol) requestAnimationFrame(step);
  };
...
document.getElementById("welcome-screen")?.addEventListener("click", startMusicOnInteraction);
...
function initParticles() {
  ...
  class Particle {
    ...
    reset(init = false) {
      ...
      this.type = Math.random() < 0.3 ? "balloon" : Math.random() < 0.5 ? "star" : "firefly";
      this.size = this.type === "balloon" ? 10 + Math.random() * 14 : 1.5 + Math.random() * 3;
      this.speedY = -(0.3 + Math.random() * 0.8);
      this.speedX = (Math.random() - 0.5) * 0.5;
      this.alpha = 0.3 + Math.random() * 0.6;
      this.hue = Math.random() < 0.5 ? 45 : 220; // gold or blue
      this.phase = Math.random() * Math.PI * 2;
      this.wobble = 0.3 + Math.random() * 0.7;
    }
    update(t) {
      this.y += this.speedY;
      this.x += this.speedX + Math.sin(t * 0.001 + this.phase) * this.wobble;
    ...
    draw() {
      ...
      if (this.type === "balloon") {
        ...
        ctx.fillStyle = this.hue === 45 ? "rgba(212,175,55,0.7)" : "rgba(45,95,204,0.7)";
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(this.x, this.y + this.size);
        ctx.lineTo(this.x, this.y + this.size + 12);
        ctx.strokeStyle = this.hue === 45 ? "rgba(212,175,55,0.4)" : "rgba(45,95,204,0.4)";
        ctx.lineWidth = 1;
        ctx.stroke();
...
function launchCelebration() {
  ...
  const colors = ["#d4af37","#f0d060","#2d5fcc","#ffffff","#a07820","#6090ff","#ffd700"];
  const pieces = [];
  for (let i = 0; i < 220; i++) {
    pieces.push({
  ...
  function draw() {
    ...
    pieces.forEach(p => {
      ...
      ctx.rotate(p.angle);
      ctx.globalAlpha = p.opacity * Math.max(0, 1 - frame / 300);
...
function launchFireworks() {
  ...
  function spawnFirework() {
    ...
    const color = Math.random() < 0.5 ? "#d4af37" : "#4d7fff";
  ...
  const spawnInterval = setInterval(() => {
    spawnFirework();
    spawned++;
    if (spawned >= 6) clearInterval(spawnInterval);
  }, 600);
  ...
  function draw() {
    ...
    if (fireworks.length > 0 || spawned < 6) {
...
function initGallery() {
  ...
  CONFIG.photos.forEach((p, i) => {
    ...
    slide.innerHTML = `
      <img src="${p.src}" alt="Photo ${i+1}" loading="lazy" onclick="openFullscreen(${i})" style="cursor:zoom-in">
      <div class="gallery-overlay"></div>
      <div class="gallery-slide-num">${i+1} / ${CONFIG.photos.length}</div>
    `;
  ...
  if (captionEl) captionEl.textContent = CONFIG.photos[0].caption;
  ...
  function goTo(n) {
    ...
    current = (n + CONFIG.photos.length) % CONFIG.photos.length;
    ...
    if (captionEl) {
      ...
      setTimeout(() => {
        captionEl.textContent = CONFIG.photos[current].caption;
  ...
  document.getElementById("gallery-prev").onclick = () => goTo(current - 1);
  document.getElementById("gallery-next").onclick = () => goTo(current + 1);
  ...
  window.openFullscreen = function(i) {
    fsImg.src = CONFIG.photos[i].src;
    fsViewer.classList.add("show");
  };
  document.getElementById("fs-close").onclick = () => fsViewer.classList.remove("show");
  fsViewer.addEventListener("click", e => {
    if (e.target === fsViewer) fsViewer.classList.remove("show");
  });
}
...
function initMessage() {
  ...
  const obs = new IntersectionObserver(entries => {
    if (entries[0].isIntersecting && !msgDone) {
      msgDone = true;
      typeMessage(el, cursor);
      obs.disconnect();
    }
  }, { threshold: 0.3 });
  if (msgCard) obs.observe(msgCard);
}
